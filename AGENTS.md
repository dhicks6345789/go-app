...
Copy index.html to ~/www/go-app, where it will be served by the system webserver. Copy the HTML API documentation generated by the Go Swaggo tool to that location (which is hosted under https://users.sansay.co.uk/d.b.hicks/go-app/) as well, making sure there is a link in index.html to it.
